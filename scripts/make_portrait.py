...
This is the generator that produces the portrait for the top of the README.
Run it once; it is not on a schedule, unlike generate_stats.py.
...
Two things decide whether the output is any good, and neither is a parameter:
  ...
  * The photo. ASCII draws with shadow, not detail — about 13 brightness levels
    in total. You need side light (a window at ~45 degrees, everything else
    off), a tight crop from chin to just above the hair, and real resolution.
    A 320px headshot fails: thin features like glasses frames are averaged
    away on downscale. Flat frontal light renders the face as a hole.
